...
INLINE ExtMove *make_promotions(ExtMove *list, Square to, Square ksq, const int Type, const int D) {
    if (Type == CAPTURES || Type == EVASIONS || Type == NON_EVASIONS) {
        ...
        if (attacks_from_knight(to) & sq_bb(ksq)) (list++)->move = make_promotion(to - D, to, KNIGHT);
    }
...
INLINE ExtMove *generate_pawn_moves(const Position *pos, ExtMove *list, Bitboard target, const Color Us,
                                    const int Type) {
    ...
    const Bitboard enemies = Type == EVASIONS ? checkers() : Type == CAPTURES ? target : pieces_c(Them);
    ...
    if (pawnsOn7 && (Type != EVASIONS || (target & TRank8BB))) {
        ...
        while (b1) list = make_promotions(list, pop_lsb(&b1), pos->st->ksq, Type, Right);

        while (b2) list = make_promotions(list, pop_lsb(&b2), pos->st->ksq, Type, Left);

        while (b3) list = make_promotions(list, pop_lsb(&b3), pos->st->ksq, Type, Up);
    }
    ...
    if (Type == CAPTURES || Type == EVASIONS || Type == NON_EVASIONS) {
        ...
        if (ep_square() != 0) {
            ...
            if (Type == EVASIONS && (target & sq_bb(ep_square() + Up))) return list;

            b1 = pawnsNotOn7 & attacks_from_pawn(ep_square(), Them);

            assert(b1);

            while (b1) (list++)->move = make_enpassant(pop_lsb(&b1), ep_square());
        }
    }
...
INLINE ExtMove *generate_moves(const Position *pos, ExtMove *list, Bitboard target, const Color Us, const int Pt,
                               const bool Checks) {
    ...
    while (bb) {
        ...
        if (Checks && (Pt == QUEEN || !(blockers_for_king(pos, !Us) & sq_bb(from)))) b &= pos->st->checkSquares[Pt];

        while (b) (list++)->move = make_move(from, pop_lsb(&b));
    }

    return list;
}

ExtMove *generate_all(const Position *pos, ExtMove *list, const Color Us, const int Type) {
    ...
    target = Type == EVASIONS ? between_bb(ksq, lsb(checkers()))
                              : Type == NON_EVASIONS ? ~pieces_c(Us) : Type == CAPTURES ? pieces_c(!Us) : ~pieces();
...
kingMoves:
    if (!Checks || blockers_for_king(pos, !Us) & sq_bb(ksq)) {
        ...
        if (Checks) b &= ~PseudoAttacks[QUEEN][square_of(!Us, KING)];

        while (b) (list++)->move = make_move(ksq, pop_lsb(&b));
...
ExtMove *generate(const Position *pos, ExtMove *list, const int Type) {
    ...
    return us == WHITE ? generate_all(pos, list, WHITE, Type) : generate_all(pos, list, BLACK, Type);
}
...
NOINLINE ExtMove __attribute__((minsize, cold)) *generate_legal(const Position *pos, ExtMove *list) {
    Color us = stm();
