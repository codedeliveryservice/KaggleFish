...
enum { NO_CASTLING = 0, WHITE_OO = 1, WHITE_OOO = 2, BLACK_OO = 4, BLACK_OOO = 8, ANY_CASTLING = 15 };
...
enum { SCALE_FACTOR_DRAW = 0, SCALE_FACTOR_NORMAL = 64, SCALE_FACTOR_MAX = 128, SCALE_FACTOR_NONE = 255 };
...
enum { DEPTH_QS_CHECKS = 0, DEPTH_QS_NO_CHECKS = -1, DEPTH_QS_RECAPTURES = -5, DEPTH_NONE = -6, DEPTH_OFFSET = -7 };
...
static const Value PieceValue[2][16] = {
...
typedef PieceToHistory CounterMoveHistoryStat[6][64];
typedef int16_t ButterflyHistory[2][4096];
typedef int16_t CapturePieceToHistory[16][64][8];

#define PAWN_CORR_HIST_SIZE 8192
typedef int16_t PawnCorrectionHistory[PAWN_CORR_HIST_SIZE][2];

#define NONPAWN_CORR_HIST_SIZE 8192
typedef int16_t NonPawnCorrectionHistory[2][2][NONPAWN_CORR_HIST_SIZE];

#define MINOR_CORR_HIST_SIZE 8192
typedef int16_t MinorCorrectionHistory[MINOR_CORR_HIST_SIZE][2];

#define MAJOR_CORR_HIST_SIZE 8192
typedef int16_t MajorCorrectionHistory[MAJOR_CORR_HIST_SIZE][2];

typedef int16_t CounterCorrectionHistory[16][64];
...
#define TEMPLATE(F, a, ...)                                                                                            \
    _Generic((a), int                                                                                                  \
             : F##_int, uint64_t                                                                                       \
             : F##_uint64_t, unsigned                                                                                  \
             : F##_unsigned, int64_t                                                                                   \
             : F##_int64_t, int16_t                                                                                    \
             : F##_int16_t, uint8_t                                                                                    \
             : F##_uint8_t, double                                                                                     \
             : F##_double)(a, __VA_ARGS__)
#else
#define TEMPLATE(F, a, ...)                                                                                            \
    _Generic((a), int                                                                                                  \
             : F##_int, uint64_t                                                                                       \
...
INLINE int piece_to_history_conv(Piece x) {
    return type_of_p(x) - !!x;
}
