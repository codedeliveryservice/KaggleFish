...
void print_pretty(Bitboard b) {
        ...
        for (int r = 7; r >= 0; r--) {
            for (int f = 0; f <= 7; f++) printf((b & sq_bb(8 * r + f)) ? "| X " : "|   ");
...
void __attribute__((minsize, cold)) bitboards_init(void) {

    for (int f = 0; f < 8; f++) FileBB[f] = f > FILE_A ? FileBB[f - 1] << 1 : FileABB;

    for (int r = 0; r < 8; r++) RankBB[r] = r > RANK_1 ? RankBB[r - 1] << 8 : Rank1BB;
    ...
    NO_UNROLL for (int c = 0; c < 2; c++)
        NO_UNROLL for (int pt = PAWN; pt <= KING; pt++)
            NO_UNROLL for (int s = 0; s < 64; s++)
                NO_UNROLL for (int i = 0; steps[pt][i]; i++) {
