...
static void init_magics(Bitboard table[], Bitboard *attacks[], Bitboard magics[], Bitboard masks[], uint8_t shifts[],
                        int deltas[], Fn index) {
    ...
    for (Square s = 0; s < 64; s++) {
        ...
        do {
            ...
            if (HasPext) attacks[s][pext(b, masks[s])] = reference[size];
        ...
        if (s < 63) attacks[s + 1] = attacks[s] + size;
