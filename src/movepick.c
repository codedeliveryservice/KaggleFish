...
static void score_quiets(const Position *pos) {
    ...
    for (ExtMove *m = st->cur; m < st->endMoves; m++) {
        ...
        Square to = move & 63;
        Square from = move >> 6;
        m->value = (*history)[c][move] + 2 * (*cmh)[0][piece_to_history_conv(piece_on(from))][to] +
                   (*fmh)[1][piece_to_history_conv(piece_on(from))][to] +
                   (*fmh2)[1][piece_to_history_conv(piece_on(from))][to] +
                   (*fmh3)[1][piece_to_history_conv(piece_on(from))][to];
    }
}
    ...
    for (ExtMove *m = st->cur; m < st->endMoves; m++)
        if (is_capture(pos, m->move))
            ...
            m->value = (*history)[c][from_to(m->move)] +
                       2 * (*cmh)[0][piece_to_history_conv(moved_piece(m->move))][to_sq(m->move)] - (1 << 28);
}
...
Move next_move(const Position *pos, bool skipQuiets) {
    ...
    case ST_GOOD_CAPTURES:
        while (st->cur < st->endMoves) {
            ...
            if (move != st->ttMove) {
                if (see_test(pos, move, -51 * (st->cur - 1)->value / 1024)) return move;
    ...
    case ST_BAD_CAPTURES:
        if (st->cur < st->endBadCaptures) return (st->cur++)->move;
        break;
    ...
    case ST_QCAPTURES:
        while (st->cur < st->endMoves) {
            move = pick_best(st->cur++, st->endMoves);
            if (move != st->ttMove && (st->depth > DEPTH_QS_RECAPTURES || to_sq(move) == st->recaptureSquare))
                return move;
        }
        if (st->depth <= DEPTH_QS_NO_CHECKS) break;
        st->cur = (st - 1)->endMoves;
    ...
    case ST_PROBCUT_2:
        while (st->cur < st->endMoves) {
            move = pick_best(st->cur++, st->endMoves);
            if (move != st->ttMove && see_test(pos, move, st->threshold)) return move;
        }
        break;
...
NOINLINE int corrhist_adjustment(const Position *pos) {
    ...
    uint64_t pawn_key  = pos->st->pawnKey     & (PAWN_CORR_HIST_SIZE - 1);
    uint64_t minor_key = pos->st->minorKey    & (MINOR_CORR_HIST_SIZE - 1);
    uint64_t major_key = pos->st->majorKey    & (MAJOR_CORR_HIST_SIZE - 1);
    ...
    int v = (*pos->pawnCorrectionHistory)[pawn_key][pos->sideToMove]           * 94
          + (*pos->minorCorrectionHistory)[minor_key][pos->sideToMove]         * 70
          + (*pos->majorCorrectionHistory)[major_key][pos->sideToMove]         * 42
          + (*pos->nonPawnCorrectionHistory)[WHITE][stm()][non_pawn_key_white] * 78
          + (*pos->nonPawnCorrectionHistory)[BLACK][stm()][non_pawn_key_black] * 78;

    Move prev = (pos->st-1)->currentMove;
    if (prev != MOVE_NONE && prev != MOVE_NULL)
        v += (*pos->counterCorrectionHistory)[piece_on(to_sq(prev))][to_sq(prev)] * 53;
