...
NOINLINE void set_check_info_noinline(Position *pos) {
    Stack *st = pos->st;
...
void print_pos(Position *pos) {
        ...
        for (int r = 7; r >= 0; r--) {
            for (int f = 0; f <= 7; f++) printf(" | %c", PieceToChar[pos->board[8 * r + f]]);
        ...
        char buf[16];
        for (Bitboard b = checkers(); b;) printf("%s ", uci_square(buf, pop_lsb(&b)));
...
void __attribute__((minsize, cold)) zob_init(void) {
...
void __attribute__((minsize, cold)) pos_set(Position *pos, char *fen) {
    ...
    if (((col = *fen++) && (col >= 'a' && col <= 'h')) && ((row = *fen++) && (row == (stm() == WHITE ? '6' : '3')))) {
        ...
        if (!(attackers_to(st->epSquare) & pieces_cp(stm(), PAWN))) st->epSquare = 0;
    } else
        st->epSquare = 0;
...
static void set_castling_right(Position *pos, Color c, Square rfrom) {
    ...
    for (Square s = min(rfrom, rto); s <= max(rfrom, rto); s++)
        if (s != kfrom && s != rfrom) pos->castlingPath[cr] |= sq_bb(s);
...
static void __attribute__((minsize, cold)) set_state(Position *pos, Stack *st) {
    ...
    for (Bitboard b = pieces(); b;) {
        ...
        if (type_of_p(pc) == KNIGHT || type_of_p(pc) == BISHOP || type_of_p(pc) == KING)
            st->minorKey ^= zob.psq[pc][s];

        if (type_of_p(pc) == ROOK || type_of_p(pc) == QUEEN || type_of_p(pc) == KING)
            st->majorKey ^= zob.psq[pc][s];
    }
}

// pos_fen() returns a FEN representation of the position.
...
void pos_fen(const Position *pos, char *str) {
    ...
    for (int r = 7; r >= 0; r--) {
        for (int f = 0; f < 8; f++) {
            for (cnt = 0; f < 8 && !piece_on(8 * r + f); f++) cnt++;
            if (cnt) *str++ = '0' + cnt;
...
Bitboard slider_blockers(const Position *pos, Bitboard sliders, Square s, Bitboard *pinners) {
    ...
    *pinners = 0;
    ...
    // snipers are removed.
    snipers =
        ((PseudoAttacks[ROOK][s] & pieces_pp(QUEEN, ROOK)) | (PseudoAttacks[BISHOP][s] & pieces_pp(QUEEN, BISHOP))) &
        sliders;
    ...
    while (snipers) {
        ...
        if (b && !more_than_one(b)) {
            blockers |= b;
            if (b & pieces_c(color_of(piece_on(s)))) *pinners |= sq_bb(sniperSq);
        }
    }
    return blockers;
}
...
bool is_legal(const Position *pos, Move m) {
    ...
    if (pieces_p(KING) & sq_bb(from)) return !(attackers_to_occ(pos, to, pieces() ^ sq_bb(from)) & pieces_c(!us));
...
bool is_pseudo_legal(const Position *pos, Move m) {
    ...
    if (!(pieces_c(us) & sq_bb(from))) return false;
    ...
    Square to = to_sq(m);
    if (pieces_c(us) & sq_bb(to)) return false;

    PieceType pt = type_of_p(piece_on(from));
    if (pt != PAWN) {
        if (type_of_m(m) != NORMAL) return false;
        switch (pt) {
        case KNIGHT:
            if (!(attacks_from_knight(from) & sq_bb(to))) return false;
            break;
        case BISHOP:
            if (!(attacks_from_bishop(from) & sq_bb(to))) return false;
            break;
        case ROOK:
            if (!(attacks_from_rook(from) & sq_bb(to))) return false;
            break;
        case QUEEN:
            if (!(attacks_from_queen(from) & sq_bb(to))) return false;
            break;
        case KING:
            if (!(attacks_from_king(from) & sq_bb(to))) return false;
            ...
            if (checkers() && (attackers_to_occ(pos, to, pieces() ^ sq_bb(from)) & pieces_c(!us))) return false;
            return true;
        default:
            assume(false);
            break;
        ...
        if (likely(type_of_m(m) == NORMAL)) {
            if (!((to + 0x08) & 0x30)) return false;
            if (!(attacks_from_pawn(from, us) & pieces_c(!us) & sq_bb(to)) &&
                !((from + pawn_push(us) == to) && is_empty(to)) &&
                !(from + 2 * pawn_push(us) == to && rank_of(from) == relative_rank(us, RANK_2) && is_empty(to) &&
                  is_empty(to - pawn_push(us))))
                return false;
    ...
    if (checkers()) {
        ...
        if (!(between_bb(square_of(us, KING), lsb(checkers())) & sq_bb(to))) return false;
    }
    return true;
}
...
bool gives_check_special(const Position *pos, Stack *st, Move m) {
    ...
    if ((blockers_for_king(pos, !stm()) & sq_bb(from)) && !aligned(m, st->ksq)) return true;
    ...
    case ENPASSANT: {
        if (st->checkSquares[PAWN] & sq_bb(to)) return true;
...
void do_move(Position *pos, const Move m, int givesCheck) {
    assert(move_is_ok(m));
    ...
    assert(is_empty(to) || color_of(piece_on(to)) == (type_of_m(m) != CASTLING ? them : us));
    ...
    if (type_of_p(piece) == ROOK || type_of_p(piece) == QUEEN || type_of_p(piece) == KING)
        st->majorKey ^= zob.psq[piece][from] ^ zob.psq[piece][to];

    // Reset en passant square
    if (unlikely((st - 1)->epSquare != 0)) key ^= zob.enpassant[file_of((st - 1)->epSquare)];
    st->epSquare = 0;
    ...
    if (likely(type_of_m(m) != CASTLING)) move_piece(pos, us, piece, from, to);
    ...
    if (type_of_p(piece) == PAWN) {
        ...
        if ((to ^ from) == 16 && (attacks_from_pawn(to ^ 8, us) & pieces_cp(them, PAWN))) {
            ...
            st->nonPawnKey[us] ^= zob.psq[promotion][to];

            if (type_of_p(promotion) == KNIGHT || type_of_p(promotion) == BISHOP)
                st->minorKey ^= zob.psq[promotion][to];

            if (type_of_p(promotion) == ROOK || type_of_p(promotion) == QUEEN)
    ...
    nnue_push(pos, m, piece, captured);
...
Key key_after(const Position *pos, Move m) {
    ...
    if (captured) k ^= zob.psq[captured][to];
...
bool see_test(const Position *pos, Move m, int value) {
    if (unlikely(type_of_m(m) != NORMAL)) return 0 >= value;
...
SMALL
bool __attribute__((minsize, cold)) is_draw(const Position *pos) {
    Stack *st = pos->st;
    ...
    if (e >= 0) {
        ...
        for (int i = 0; i <= e; i += 2) {
            stp -= 2;
            if (stp->key == st->key) return true; // Draw at first repetition
        }
    }
...
static int pos_is_ok(Position *pos, int *failedStep) {
    ...
    if (step == King)
      if (   std::count(board, board + SQUARE_NB, W_KING) != 1
        ...
        if (step == Bitboards) {
            if ((pieces_c(WHITE) & pieces_c(BLACK)) || (pieces_c(WHITE) | pieces_c(BLACK)) != pieces()) return 0;

            for (int p1 = PAWN; p1 <= KING; p1++)
                for (int p2 = PAWN; p2 <= KING; p2++)
                    if (p1 != p2 && (pieces_p(p1) & pieces_p(p2))) return 0;
        }

        if (step == StackOK) {
            Stack si = *(pos->st);
            set_state(pos, &si);
            if (memcmp(&si, pos->st, StateSize)) return 0;
        }

        if (step == Lists)
            for (int c = 0; c < 2; c++)
                for (int pt = PAWN; pt <= KING; pt++)
                    if (piece_count(c, pt) != popcount(pieces_cp(c, pt))) return 0;

        if (step == Castling)
            for (int c = 0; c < 2; c++)
                for (int s = 0; s < 2; s++) {
                    int cr = make_castling_right(c, s);
                    if (!can_castle_cr(cr)) continue;
